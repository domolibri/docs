...
├── EditoraId
├── Nome, Email, Telefone
├── TipoVinculo (Interno, Freelancer, Autor, Sistema)
...
- Segregação de acesso por `EditoraId`
...
2. POST /api/auth/register cria:
   ├── Editora
   ├── UsuarioEditora (primeiro administrador)
   ├── Roles iniciais padrão
   └── Configurações iniciais do tenant
3. Usuário realiza login.
4. A autenticação gera JWT com EditoraId, UsuarioId, Roles e claims de permissão.
5. Frontend carrega dashboard inicial, branding e configurações básicas da editora.
6. Admin conclui setup inicial: logo, cor primária, serviços habilitados, dados financeiros e preferências operacionais.
```
...
2. GET /api/usuarios retorna apenas usuários do tenant corrente.
3. Admin cadastra ou convida novos usuários.
4. POST /api/usuarios ou POST /api/usuarios/convites cria usuário com vínculo à EditoraId.
5. Admin atribui roles e permissões.
6. Usuário convidado ativa a conta e passa a acessar apenas os módulos compatíveis com seu perfil.
7. Todas as alterações críticas são registradas em AuditLog.
```
...
4. Participantes do livro são associados:
   ├── Autor
   ├── Revisor
   ├── Capista
   ├── Diagramador
...
3. Uma etapa pode ser:
   ...
   ├── atribuída a parceiro/freelancer,
   └── executada por IA.
4. O executor recebe notificação e acessa sua fila de trabalho.
...
3. Criar o onboarding da editora: cadastro da `Editora`, criação automática do primeiro usuário administrador, login, dashboard inicial e branding básico.
4. Estruturar o modelo de identidade com `UsuarioEditora`, `Role`, permissões granulares, trilha de auditoria e logs administrativos.

### **Fase 2: Gestão de Usuários, Roles e Atores Editoriais (2–4 semanas)**

5. Implementar CRUD de usuários da editora: cadastro manual, convite, ativação, inativação, redefinição de acesso e controle de vínculo com a editora.
...
7. Implementar matriz de autorização por role e por escopo, garantindo que cada ator visualize apenas os livros, etapas, chats, artefatos e rotinas financeiras que lhe cabem.
8. Criar telas de gestão de usuários, perfis, permissões e associação dos usuários aos fluxos editoriais.
...
11. Permitir associar atores editoriais ao livro e às etapas da esteira, incluindo colaboradores internos e parceiros externos.
